...
Tickscope は、あらゆる MCP クライアント（Claude Code、Cursor、Codex、Gemini CLI など）に**リアルタイムおよびヒストリカルな暗号資産マーケットデータを無料で**提供する、セルフホスト可能な [Model Context Protocol](https://modelcontextprotocol.io) サーバーです。バックグラウンドで取引所の WebSocket 接続を温めておくことで、エージェントは**サブ秒の鮮度**を持つ価格をそのまま読み取れます。同じサーバーが **73 種類のテクニカル指標**と**チャート構造の認識**まで提供し、API キーも不要です。
...
取引エージェントは爆発的に増えていますが、その土台となるデータ層は、いまだに断片化し、REST ポーリングに限られ、有料であることも少なくありません。Tickscope はそうしたエージェントに、**リアルタイムで無料のマーケットデータ**を 1 つのサーバーから提供します —— 複数の取引所、API キー不要。
...
ライブの Binance/Bybit/OKX に対するカラー端末のウォークスルーです —— コールド→ウォームの鮮度（REST → WebSocket）、シグナル付きの指標、ダイバージェンス、マーケット構造、サポート/レジスタンス。このウォークスルーを冒頭の GIF として収録する方法は [examples/RECORDING.md](examples/RECORDING.md) を参照してください。
...
[ccxt](https://github.com/ccxt/ccxt) が対応する取引所はすべて `TICKSCOPE_EXCHANGES` で有効化できます。公開データのみ —— キー不要。
...
| `deep_analyze` | マルチタイムフレームでまとめて読む：トレンドの一致度、相場状態のコンテキスト、ヒストリカルなシグナル実績、そして統合した結論まで |
...
仕様は `"name:p1,p2"` 形式で、**Pine Script 構文** —— `ta.rsi(14)`、`ta.ema(20)`、`ta.wt(10,21)`、`ta.sqz` —— も受け付けるため、TradingView ユーザーは見慣れた表現をそのまま貼り付けられます。派生シグナルには、買われすぎ/売られすぎの状態、MACD/PPO/WaveTrend/QQE のクロス、オシレーターのゼロラインクロス、Supertrend/PSAR の direction & flip、squeeze の on/off、DMI/Heikin-Ashi のトレンド、Ichimoku の雲の位置が含まれます。暗号資産/Pine の定番（WaveTrend、TTM Squeeze、QQE、Connors RSI、Schaff Trend Cycle、VIDYA、T3）を内蔵。新しい指標の追加は `REGISTRY` への 1 行の宣言だけです。

### 構造認識

Tickscope は数値指標に加えて、*いまチャートで起きていること*まで記述します。`detect_patterns` はローソク足パターン（包み足、ハンマー/首吊り線、十字線系、明けの明星/宵の明星、赤三兵/黒三兵 など）をバイアス付きで命名し、`analyze_structure` は HH/HL/LH/LL でラベル付けしたスイング高値・安値、推定トレンド、Break-of-Structure / Change-of-Character イベント（SMC スタイル）を返し、`find_support_resistance` はスイング・ピボットをサポート/レジスタンス帯に集約してタッチ回数を数えます。これにより、エージェントはトレーダーの言葉でチャートを*語れる*ようになります。

### ディープ分析

`deep_analyze` は 1 つのシンボルを 1 回の呼び出しで多角的に読み取り、次の 4 つを返します。エージェントが何種類ものツールを連鎖的に呼び出す必要はありません。

- **マルチタイムフレームのトレンド一致度：** 同じシンボルを 1d/4h/1h のラダーで読み、各タイムフレームが揃っているか食い違っているかを報告します。
- **相場状態のコンテキスト：** 価格が直近レンジのどこにいるか（パーセンタイル）、トレンドの状態（`trending_up` / `trending_down` / `ranging`、ADX と Kaufman 効率比から算出）、ボラティリティの状態（ATR パーセンタイル）。これで「RSI 30」のような数値を、それが出てきた状況に照らして解釈できます。
- **ヒストリカルなシグナル実績：** いま出ているダイバージェンスについて、同じシンボル/タイムフレームで過去に確定したすべての事例について、フォワードリターンの分布（件数、勝率、中央値）を示します。先読みもリペイントもない、厳密に因果的なイベントスタディです。
- **統合した結論：** バイアス、確信度、タイムフレームの一致、執行タイムフレームの相場状態、そして明示的な注意書き。Python 側で決定的に算出するので、モデルが生の数値を目で追って判断がブレることはありません。

軽量な `compute_indicators` も同じ相場状態のコンテキストをインラインで返すようになりました（追加コストはほぼゼロ）。シグナル履歴は確定足ごとにメモ化されるため、ウォームな読み取りは高速なままです。MCP プロンプト対応のクライアントでは、スラッシュコマンド `/mcp__tickscope__deep_analyze`（シンボルとタイムフレームを指定）としても公開され、ユーザーがフル分析をその場で起動できます。

### リソース

対応クライアントは、ライブ状態を MCP リソースとして読み取れます：`tickscope://status`、`tickscope://watched`、テンプレート `tickscope://ticker/{exchange}/{symbol}`。

## プロンプト例

- 「Binance の BTC/USDT、いまいくら？24 時間の変化率は？」
- 「この SOL/USDT のセットアップ、入る価値ある？マルチタイムフレームで深く分析して。」
- 「BTC/USDT の 4 時間足に RSI ダイバージェンスある？あるなら、この銘柄で過去にそのシグナルがどうだったかも教えて。」
- 「出来高上位 50 の USDT ペアから、RSI が 30 未満のものをスクリーニングして。」
- 「ETH/USDT 4 時間足の主要なサポート/レジスタンスを示して、構造が今崩れたか（BOS/CHoCH）も見て。」
- 「Bybit の BTC 無期限、いまのファンディングレートは？過熱してる？」
- 「BTC/USDT を Binance・Bybit・OKX で比較して、裁定スプレッドを見せて。」

より多くの例は [`examples/demo_prompts.md`](examples/demo_prompts.md) のレシピ集を参照してください（ディープ分析、シグナルのヒストリカル検証、スクリーニング、モニタリング、戦略チェックなど）。
...
| 変数 | デフォルト | 説明 |
|---|---|---|
| `TICKSCOPE_EXCHANGES` | `binance,bybit,okx` | 有効な取引所（カンマ区切り） |
| `TICKSCOPE_DEFAULT_EXCHANGE` | `binance` | `exchange` 省略時のデフォルト |
| `TICKSCOPE_MAX_WATCHED_SYMBOLS` | `25` | 同時 WS 購読の上限（超過時は LRU で解放） |
| `TICKSCOPE_RING_BUFFER_SIZE` | `1000` | シンボルごとの約定バッファサイズ |
| `TICKSCOPE_OHLCV_CACHE_PATH` | `~/.tickscope/ohlcv.duckdb` | DuckDB キャッシュファイル |
| `TICKSCOPE_OHLCV_CACHE_TTL_S` | `60` | 最新ローソク足の鮮度ウィンドウ |
| `TICKSCOPE_REST_RETRIES` | `3` | 一時的な REST エラー（レート制限/ネットワーク）の再試行回数 |
| `TICKSCOPE_SCREEN_CONCURRENCY` | `5` | スクリーニング/集約時の同時シンボル数 |
| `TICKSCOPE_TRANSPORT` | `stdio` | `stdio` または `http` |
| `TICKSCOPE_LOG_LEVEL` | `INFO` | ログレベル |
