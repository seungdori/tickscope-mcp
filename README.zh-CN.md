...
一个连接 Binance/Bybit/OKX 实时数据的彩色终端演示 —— 从冷启动到热连接的数据新鲜度变化（REST → WebSocket）、带信号的指标、背离、市场结构以及支撑/阻力。将其制作成上方 GIF 的方法见 [examples/RECORDING.md](examples/RECORDING.md)。
...
| `deep_analyze` | 多周期综合解读：趋势共振 + 市场状态背景 + 历史信号表现，最后给出一个综合判定 |
...
每个行情响应都包含 `source`（`websocket`|`rest`）、`age_ms` 和 `timestamp`，因此随时都能核对数据是否新鲜。
...
规格为 `"name:p1,p2"` 格式，同时也接受 **Pine Script 语法** —— `ta.rsi(14)`、`ta.ema(20)`、`ta.wt(10,21)`、`ta.sqz` —— 因此 TradingView 用户可以直接粘贴熟悉的表达式。衍生信号包括超买/超卖状态、MACD/PPO/WaveTrend/QQE 交叉、振荡器的零轴交叉、Supertrend/PSAR 方向与翻转、squeeze 开/关、DMI/Heikin-Ashi 趋势，以及 Ichimoku 云图位置。内置加密/Pine 热门指标（WaveTrend、TTM Squeeze、QQE、Connors RSI、Schaff Trend Cycle、VIDYA、T3）。新增一个指标只需在 `REGISTRY` 中声明一行。
...
`deep_analyze` 一次调用就能把一个标的看透，省去让智能体自己串联多个工具的麻烦：

- **多周期趋势共振：** 在 1d/4h/1h 三档周期上读取同一标的，告诉你各周期是彼此印证还是相互打架。
- **市场状态背景：** 价格在近期区间中的位置（百分位）、趋势状态（`trending_up` / `trending_down` / `ranging`，由 ADX + Kaufman 效率比得出），以及波动率状态（由 ATR 百分位得出）—— 有了这些，"RSI 30" 这种数值才能放回它所处的环境里去看。
