...
트레이딩 에이전트는 폭발적으로 늘고 있지만, 정작 바탕이 되는 데이터 계층은 여전히 파편화돼 있고 REST 폴링만 되는 데다 유료인 경우가 많습니다. TickFeed는 이런 에이전트에 **실시간 무료 시세 데이터**를 단일 서버로 제공합니다 — 멀티 거래소, API 키 불필요.
...
[ccxt](https://github.com/ccxt/ccxt)가 지원하는 어떤 거래소든 `TICKFEED_EXCHANGES`로 활성화할 수 있습니다. 공개 데이터만 사용 — 키 불필요.
...
지원하는 클라이언트는 라이브 상태를 MCP 리소스로 읽을 수 있습니다: `tickfeed://status`, `tickfeed://watched`, 그리고 템플릿 `tickfeed://ticker/{exchange}/{symbol}`.
...
| `TICKFEED_DEFAULT_EXCHANGE` | `binance` | `exchange` 생략 시 기본값 |
| `TICKFEED_MAX_WATCHED_SYMBOLS` | `25` | 동시 WS 구독 상한 (초과 시 LRU 해제) |
| `TICKFEED_RING_BUFFER_SIZE` | `1000` | 심볼당 체결 버퍼 크기 |
| `TICKFEED_OHLCV_CACHE_PATH` | `~/.tickfeed/ohlcv.duckdb` | DuckDB 캐시 파일 |
| `TICKFEED_OHLCV_CACHE_TTL_S` | `60` | 최신 캔들 신선도 윈도우 |
| `TICKFEED_REST_RETRIES` | `3` | 일시적 REST 오류(레이트리밋/네트워크) 재시도 횟수 |
| `TICKFEED_SCREEN_CONCURRENCY` | `5` | 스크리닝/집계 시 동시 심볼 수 |
| `TICKFEED_TRANSPORT` | `stdio` | `stdio` 또는 `http` |
